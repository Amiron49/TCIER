...
public class SlotLimitedEquipperUI : MonoBehaviour
{
	...
	public StatsDisplayUI propertiesDisplayUI;
	...
	public void Refresh()
	{
		...
		for (var index = 0; index < fullList.Count; index++)
		{
			...
			unEquipItemController.OnHoverStart += (_, _) =>
			{
				...
				propertiesDisplayUI.DisplayPreview(currentProperties, properties, _slotLimitedEquipper.IsPositiveChangeMap);
			};

			unEquipItemController.OnHoverEnd += (_, _) =>
			{
				RemoveXMarks();
				propertiesDisplayUI.DisplayNormal(_slotLimitedEquipper.Properties);
			};
